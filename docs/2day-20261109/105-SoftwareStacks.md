...
<figure markdown style="border: 1px solid #000">
  ![Software stack design considerations](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/SoftwareStacksDesignConsiderations.png){ loading=lazy }
</figure>
...
<figure markdown style="border: 1px solid #000">
  ![The LUMI solution](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/SoftwareStacksTheLUMISolution.png){ loading=lazy }
</figure>
...
<figure markdown style="border: 1px solid #000">
  ![Policies](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/SoftwareStacksPolicies.png){ loading=lazy }
</figure>
...
<figure markdown style="border: 1px solid #000">
  ![Organisation"Software Stacks](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/SoftwareStacksOrganisation.png){ loading=lazy }
</figure>
...
Currently there are four such stacks on LUMI or planned to be come available soon:
...
-   `Local-CSC`: Enables software installed and maintained by CSC. 
    ...
    restricted by their license 
    or only useful to users of other CSC services (e.g., the allas module).
    ...
    used on the Finnish national systems and 
    [their documentation](https://docs.csc.fi/apps/by_availability/) 
    is also joint with that of the Finnish national systems, with a list of
    [packages offered as regular LUMI modules](https://docs.csc.fi/apps/by_availability/#lumi)
    and a list of
    [packages offered via the LUMI web interace (Open OnDemand)](https://docs.csc.fi/apps/by_availability/#lumi-web-interface).

-   The [LUMI AI Factory containers](https://docs.lumi-supercomputer.eu/laif/software/ai-environment/) 
    are another local offering. They take over the responsibility of AI-focused containers
    from both CSC and LUST, so AI containers offered by CSC or LUST will no longer receive
    updates.

-   `Local-quantum` contains some packages of general use, but also some packages that 
    are only relevant to Finnish researchers with an account on the 
    [VTT quantum computers](https://docs.csc.fi/computing/quantum-computing/overview/). 
...
<figure markdown style="border: 1px solid #000">
  ![Accessing the Cray PE on LUMI slide 1](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/SoftwareStacksAccessingCrayPE_1.png){ loading=lazy }
</figure>
...
And note that the base image of the OS is deliberately kept small for faster boot times of compute nodes
and as it is on a RAM disk, hence reducing RAM capacity available to applications. So we will not install
additional software in that environment.
...
Environment **works exactly as you'd expect from this course or the advanced courses
that LUST organises**.
In this environment we will only install software that is compatible with all compilers on the system and
has very few dependencies, and can live outside the system directories.

??? Note "How do we manage software in CrayEnv?"
    To be able to rebuild the system efficiently after an accident, software in this stack
    is managed through EasyBuild though we do not support direct use of EasyBuild in the
    CrayEnv environment. Instead we have a way of injecting software from the LUMI partition
    into CrayEnv. This may impose some additional restrictions on software that can be 
    installed in the CrayEnv stack, but guarantees better reproducibility and faster repair
    should the stack get damaged.


#### LUMI stack

<figure markdown style="border: 1px solid #000">
  ![Accessing the Cray PE on LUMI slide 2](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/SoftwareStacksAccessingCrayPE_2.png){ loading=lazy }
</figure>
...
easily due to the way system administration on a Cray system is done. They will load the matching
PrgEnv module also, but do so in a more controlled environment to guarantee that the correct
...
<figure markdown style="border: 1px solid #000">
  ![Accessing the Cray PE on LUMI slide 3](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/SoftwareStacksAccessingCrayPE_3.png){ loading=lazy }
</figure>

To manage the heterogeneity in the hardware, the LUMI software stack uses **two levels of modules**

First there are the **LUMI/25.09, LUMI/25.03, LUMI/24.03 and LUMI/23.09 modules**. 
At times there may even be more versions offered through the 
[ccpe containers](https://lumi-supercomputer.github.io/LUMI-EasyBuild-docs/c/ccpe/)
with versions of the Cray Programming Environment that are not on the system.
...
And there are some other hidden partitions that are used to inject software in places that are also
available outside the LUMI software stack but managed through EasyBuild.
...
!!! Note "Supported stacks after the January 2026 system update"
    The best supported stack is currently the 25.03 stack as that version of the Cray Programming
    Environment is also the default and the intended to be used with the other upgrades system 
    components. We also get full support from HPE for the 25.09 version of the Cray Programming 
    Environment and made basic libraries and some additional software available for that stack.
    Note though that when using software in that stack that uses 
    [Cray MPI](102-CPE.md#cray-mpi) or any of the 
    [scientific and math libraries](102-CPE.md#scientific-and-math-libraries), you will have to take
    care of
    [adjusting LD_LIBRARY_PATH as discussed at the end of the CPE chapter](102-CPE.md#warning-1-you-do-not-always-get-what-you-expect).

    24.03 still functions reasonably well, especially for CPU-only software. Some GPU software will
    still compile or build but you will have to use a newer version or ROCm(tm) as intended for 
    that version of the PE.
    
    There are known issues however with 23.09 even for CPU software (e.g., there is no compatible
    MPI library for the GNU compilers that functions with the current network stack), 
    and it was never designed to be used with ROCm(tm) 6.
...
<figure markdown style="border: 1px solid #000">
  ![Installing software on HPC systems](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildInstallingSoftwareHPC.png){ loading=lazy }
</figure>
...
<figure markdown style="border: 1px solid #000">
  ![Extending the LUMI stack with EasyBuild](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildExtendingLUMIStack.png){ loading=lazy }
</figure>
...
-   The Intel-based toolchains have their problems also. At the moment, the Intel compilers with the
    ...
    very recent versions of MKL are less AMD-hostile.

    We cannot get upstream support for these compilers so LUST cannot fix those issues.
...
<figure markdown style="border: 1px solid #000">
  ![EasyBuild recipes - easyconfigs](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildEasyConfig.png){ loading=lazy }
</figure>
...
<figure markdown style="border: 1px solid #000">
  ![The toolchain concept](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildToolchain_1.png){ loading=lazy }
</figure>
...
| `PrgEnv-gnu`  | `cpeGNU`       | GNU C/C++ and Fortran                           |
| `PrgEnv-aocc` | `cpeAOCC`      | AMD CPU compilers (login nodes and LUMI-C only) |
| `PrgEnv-amd`  | `cpeAMD`       | AMD ROCm(tm) GPU compilers (LUMI-G only)            |


<figure markdown style="border: 1px solid #000">
  ![The toolchain concept 2](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildToolchain_2.png){ loading=lazy }
</figure>
...
<figure markdown style="border: 1px solid #000">
  ![easyconfig names and module names](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyConfigModuleNames.png){ loading=lazy }
</figure>
...
Consider, e.g., the easyconfig file `GROMACS-2024.3-cpeGNU-25.03-PLUMED-2.9.4-noPython-CPU.eb`.
...
<figure markdown style="border: 1px solid #000">
  ![Installing: Where to install](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildInstallingStep1.png){ loading=lazy }
</figure>
...
by the `LUMI` - or actually **the `partition` - module in the LUMI stack to find software** 
so all users in your project who want to use the software should set that variable.


#### Step 2: Configure the environment

<figure markdown style="border: 1px solid #000">
  ![Installing: Configure the environment](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildInstallingStep2_1.png){ loading=lazy }
</figure>
...
<figure markdown style="border: 1px solid #000">
  ![Step 2: Configure the environment - Demo](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildInstallingStep2_2.png){ loading=lazy }
</figure>
...
<figure markdown style="border: 1px solid #000">
  ![Installing: Install the software](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildInstallingStep3.png){ loading=lazy }
</figure>
...
!!! Note "Results of the searches:"
    ...
    <figure markdown style="border: 1px solid #000">
      ![GROMACS in the LUMI Software Library](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildGROMACSSoftLib.png){ loading=lazy }
    </figure>

    `eb --search GROMACS` produces:

    <figure markdown style="border: 1px solid #000">
      ![eb --search GROMACS](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildGROMACSSearch_1.png){ loading=lazy }
    </figure>

    while `eb -S GROMACS` produces:

    <figure markdown style="border: 1px solid #000">
      ![eb -S GROMACS](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildGROMACSSearch_2.png){ loading=lazy }
    </figure>
...
from `LUMI/25.03`, and a build meant for CPU-only systems. The `-CPU` extension is not
...
```bash
eb GROMACS-2024.3-cpeGNU-25.03-PLUMED-2.9.4-noPython-CPU.eb –Dr
```

The `-Dr` flag tells EasyBuild to just perform a check for all the dependencies that are needed
when installing this package. Note that this is something that has changed in LUMI/25.03.
Before, using `-D` was enough, but in EasyBuild 5, this only shows the direct dependencies,
and not the dependencies of the dependencies, etc., and the list of uninstalled dependencies
can hence be incomplete.

!!! Demo "The output of this command looks like:"

    <figure markdown style="border: 1px solid #000">
      ![eb GROMACS-2024.3-cpeGNU-24.03-PLUMED-2.9.4-noPython-CPU.eb –D](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildGROMACSDep_01.png){ loading=lazy }
    </figure>

    <figure markdown style="border: 1px solid #000">
      ![eb GROMACS-2024.3-cpeGNU-24.03-PLUMED-2.9.4-noPython-CPU.eb –D (2)](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildGROMACSDep_02.png){ loading=lazy }
    </figure>
...
!!! Demo "Running EasyBuild to install GROMACS and dependency"
    ...
    <figure markdown style="border: 1px solid #000">
      ![eb GROMACS-2024.3-cpeGNU-25.03-PLUMED-2.9.4-noPython-CPU.eb -r](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildGROMACSInst_01.png){ loading=lazy }
    </figure>
    ...
    <figure markdown style="border: 1px solid #000">
      ![eb GROMACS-2024.3-cpeGNU-25.03-PLUMED-2.9.4-noPython-CPU.eb -r (2)](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildGROMACSInst_02.png){ loading=lazy }
    </figure>
    ...
    MPI, so it will do 4 iterations.

    <figure markdown style="border: 1px solid #000">
      ![eb GROMACS-2024.3-cpeGNU-2503-PLUMED-2.9.4-noPython-CPU.eb -r (3)](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildGROMACSInst_03.png){ loading=lazy }
    </figure>

    <figure markdown style="border: 1px solid #000">
      ![eb GROMACS-2024.3-cpeGNU-2503-PLUMED-2.9.4-noPython-CPU.eb -r (4)](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildGROMACSInst_04.png){ loading=lazy }
    </figure>

    <figure markdown style="border: 1px solid #000">
      ![eb GROMACS-2024.3-cpeGNU-2503-PLUMED-2.9.4-noPython-CPU.eb -r (5)](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildGROMACSInst_05.png){ loading=lazy }
    </figure>

    <figure markdown style="border: 1px solid #000">
      ![eb GROMACS-2024.3-cpeGNU-2503-PLUMED-2.9.4-noPython-CPU.eb -r (6)](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildGROMACSInst_06.png){ loading=lazy }
    </figure>
...
<figure markdown style="border: 1px solid #000">
  ![Installing: Install the software](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildInstallingStep3Note.png){ loading=lazy }
</figure>
...
<figure markdown style="border: 1px solid #000">
  ![More advanced work](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildAdvanced_1.png){ loading=lazy }
</figure>
...
for instance, for [VASP](https://lumi-supercomputer.github.io/LUMI-EasyBuild-docs/v/VASP/). 
LUMI would likely be in violation of the license if it would 
...
directory run EasyBuild, for instance for VASP 6.5.01 with the GNU compilers:
```bash
eb VASP-6.5.1-cpeGNU-25.03-build02.eb –r . 
```
or
```bash
eb VASP-6.5.1-cpeGNU-25.03-build02.eb --sourcepath . 
```
(Both versions are not fully equivalent: `-r` tells EasyBuild to use that directory to search for
dependencies, but it just happens that these directories are also used to check for source files,
while `--sourcepath` explicitly tells EasyBuild to search in that directory for source files.)


### More advanced work (2): Repositories

<figure markdown style="border: 1px solid #000">
  ![More advanced work (2): Repositories](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildAdvanced_2.png){ loading=lazy }
</figure>
...
<figure markdown style="border: 1px solid #000">
  ![More advanced work (3): Reproducibility](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildAdvanced_3.png){ loading=lazy }
</figure>
...
<figure markdown style="border: 1px solid #000">
  ![EasyBuild tips and tricks](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildTipsTricks.png){ loading=lazy }
</figure>
...
<figure markdown style="border: 1px solid #000">
  ![EasyBuild training](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/EasyBuildTraining.png){ loading=lazy }
</figure>
...
<figure markdown style="border: 1px solid #000">
  ![Software stacks on LUMI](https://462000265.lumidata.eu/2day-20261109/img/LUMI-2day-20261109-105-SoftwareStacks/DocsLocalStacks.png){ loading=lazy }
</figure>
